// apps/web/src/features/auth/pages/SignupOwner.tsx
...
export function SignupOwner({ onSignedUp, onGoToLogin }: Props) {
  ...
  const organizationNameError = useMemo(() => {
    if (!touched.organizationName) return null;
    if (!organizationNameTrimmed) return "Organization name is required";
    if (organizationNameTrimmed.length < 2) {
      return "Organization name must be at least 2 characters";
    }
    return null;
  }, [organizationNameTrimmed, touched.organizationName]);
  ...
  const canSubmit = useMemo(() => {
    ...
    if (organizationNameTrimmed.length < 2) return false;
    if (!isValidEmail(emailTrimmed)) return false;
    if (password.length < 6) return false;
    return true;
  }, [loading, nameTrimmed, organizationNameTrimmed, emailTrimmed, password]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      await signupOwner({
        ...
        email: emailTrimmed,
        password,
        organizationName: organizationNameTrimmed,
      });
        ...
        <div className="w-full max-w-md">
          <div className="mb-6 text-center">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Create your account and your organization in one step.
            </p>
          </div>
          ...
          <Card className="p-5 sm:p-6">
            {error && (
              <div
                ...
                aria-live="polite"
              >
                <div className="font-medium text-red-900">Could not create workspace</div>
              ...
              <div>
                ...
                <div className="mt-2">
                  <Input
                    ...
                    onBlur={() =>
                      setTouched((t) => ({ ...t, organizationName: true }))
                    }
                    placeholder="Acme Inc"
                    autoComplete="organization"
                    aria-invalid={Boolean(organizationNameError)}
                    className={
                      organizationNameError ? "border-red-300 focus:ring-red-500" : undefined
                    }
                  />
                </div>
                <div className="mt-2 min-h-[16px] text-xs">
                  {organizationNameError ? (
                    <span className="text-red-700">{organizationNameError}</span>
                  ) : touched.organizationName && organizationNameTrimmed.length >= 2 ? (
                    <span className="text-gray-500">Looks good</span>
                  ) : (
              ...
              <div>
                ...
                <div className="mt-2 min-h-[16px] text-xs">
                  {emailError ? (
                    ...
                    <span className="text-gray-500">
                      Use the email you want for your account.
                    </span>
                  )}
                </div>
              </div>
